...
use crate::config::{CUTTING_TIME, ASSEMBLY_TIME, PACKAGING_TIME, SIMULATION_SPEED_MULTIPLIER, ENABLE_VERBOSE_LOGGING};
use std::thread;
...
pub fn start_station(
    ...
    thread::spawn(move || {
        ...
        if ENABLE_VERBOSE_LOGGING {
            println!("[{}] Iniciada - Tiempo procesamiento: {}s", name, processing_time);
        }
        ...
        loop {
            match input_rx.recv() {
                Ok(mut product) => {
                    ...
                    let adjusted_time = (processing_time as f64 / SIMULATION_SPEED_MULTIPLIER) as u64;
                    ...
                    if ENABLE_VERBOSE_LOGGING {
                        println!("[{}] Producto {} procesado en {}s", 
                            name, product.id, current_timestamp() - entry_time);
                    }
